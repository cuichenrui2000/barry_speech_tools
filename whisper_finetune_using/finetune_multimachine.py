...
add_arg("warmup_steps",  type=int, default=2048,      help="训练预热步数")
...
add_arg("use_tar_file_datalist", type=bool, default=False,    help="是否使用 tarfile 读取 datalist")
args = parser.parse_args()
print_arguments(args)

if args.resume_from_checkpoint:
    # 获取Whisper的数据处理器，这个包含了特征提取器、tokenizer
    processor = WhisperProcessor.from_pretrained(args.resume_from_checkpoint,
...
# 读取数据
if args.use_tar_file_datalist:
    tarfile_datalist = TarFileDataset(data_list_path=args.train_data)
    ...
    train_dataset = CustomDataset(data_list_path=args.train_data,
                                ...
                                tarfile_datalist=tarfile_datalist,
                                use_tar_file_datalist=args.use_tar_file_datalist)
else:
    train_dataset = CustomDataset(data_list_path=args.train_data,
                                processor=processor,
                                language=args.language,
...
if args.resume_from_checkpoint:
    # 获取模型
    model = WhisperForConditionalGeneration.from_pretrained(args.resume_from_checkpoint,
...
training_args = \
    Seq2SeqTrainingArguments(output_dir=output_dir,  # 保存检查点和意志的目录
                             ...
                             save_total_limit=20,  # 只保存最新检查点的数量
