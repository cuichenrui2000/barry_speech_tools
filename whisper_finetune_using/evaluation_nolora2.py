...
add_arg("test_data",   type=str, default="/nfs/volume-225-14/laizhihao_i/data/testset_all/Chinese-English_Mix/Chinese-English_Mix.jsonl",            help="测试集的路径")
add_arg("model_path",  type=str, default="/nfs/volume-225-14/laizhihao_i/Whisper/Whisper-Finetune/checkpoint/2w_data/model_medium/checkpoint-85000", help="合并模型的路径，或者是huggingface上模型的名称")
...
# 开始评估
hyp_path = "/nfs/volume-225-14/laizhihao_i/Whisper/Whisper-Finetune/checkpoint/2w_data/model_medium/result/hyp_Chinese-English_Mix"
lab_path = "/nfs/volume-225-14/laizhihao_i/Whisper/Whisper-Finetune/checkpoint/2w_data/model_medium/result/lab_Chinese-English_Mix"
        ...
        with torch.cuda.amp.autocast():
            with torch.no_grad():
                ...
                for i in range(len(decoded_preds)):
                    f1.write(str(step) + '_' + str(i) + "\t" + decoded_preds[i] + '\n')
                    f2.write(str(step) + '_' + str(i) + "\t" + decoded_labels[i] + '\n')
...
# parser = argparse.ArgumentParser(description=__doc__)
# add_arg = functools.partial(add_arguments, argparser=parser)
# add_arg("test_data",   type=str, default="/nfs/volume-225-14/laizhihao_i/data/filter_data/new_testset/testset",            help="测试集的路径")
# add_arg("model_path",  type=str, default="/nfs/volume-225-14/laizhihao_i/Whisper/Whisper-Finetune/checkpoint/2w_data/model_medium/checkpoint-75000", help="合并模型的路径，或者是huggingface上模型的名称")
# add_arg("batch_size",  type=int, default=16,        help="评估的batch size")
# add_arg("num_workers", type=int, default=8,         help="读取数据的线程数量")
# add_arg("language",    type=str, default="Chinese", help="设置语言，可全称也可简写，如果为None则评估的是多语言")
# add_arg("remove_pun",  type=bool, default=True,     help="是否移除标点符号")
...
# # 获取测试数据
# for file_name in os.listdir(args.test_data):
#     test_data_path = os.path.join(args.test_data, file_name)
#     test_dataset = CustomDataset(data_list_path=test_data_path,
#                                 processor=processor,
#                                 timestamps=args.timestamps,
#                                 min_duration=args.min_audio_len,
#                                 max_duration=args.max_audio_len)
...
#     hyp_path = "/nfs/volume-225-14/laizhihao_i/Whisper/Whisper-Finetune/checkpoint/2w_data/model_medium/result/hyp_" + write_name
#     lab_path = "/nfs/volume-225-14/laizhihao_i/Whisper/Whisper-Finetune/checkpoint/2w_data/model_medium/result/lab_" + write_name
#     with open(hyp_path, "w") as f1, open(lab_path, "w") as f2:
#         for step, batch in enumerate(tqdm(eval_dataloader)):
#             with torch.cuda.amp.autocast():
#                 with torch.no_grad():
